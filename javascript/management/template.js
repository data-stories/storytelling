...
function getRuleBasedRecommendations(container) {
  ...
  if((pStr === "") && !/^I/.test(nStr)) {
    var recArr = [new TextBlock("Introduce your article in a single paragraph, highlighting key questions and findings.", "Introduction")];
    recSetArr.push(createRecBlock("Add missing article introduction", recArr));
  }
  ...
  if(!/Z$/.test(pStr) && (nStr === "") && (pStr.split('C').length-1 >= MAX_CLAIMS)) {
    ...
    recSetArr.push(createRecBlock("Add missing article conclusion - at least "+MAX_CLAIMS+" article claims made", recArr));
  }
  ...
  if(/[IO]$/.test(pStr)) {
    ...
    recSetArr.push(createRecBlock("Present a new claim with supporting facts", recArr));
  }
  ...
  if(/[CFX]$/.test(pStr)) {
    ...
    recSetArr.push(createRecBlock("Add additional fact/evidence to justify this claim", recArr));
  }
  ...
  if(/F$/.test(pStr)) {
    ...
    recSetArr.push(createRecBlock("Explain nature of fact/evidence just presented", recArr));
  }
  ...
  if(!/Z$/.test(pStr) && (nStr === "") && (pStr.split('C').length-1 >= 1) && (pStr.split('C').length-1 < MAX_CLAIMS)) {
    var recArr = [new TextBlock("Provide a lead-out summary of the main conclusions in the article.", "Article conclusion")];
    recSetArr.push(createRecBlock("Add an article conclusion", recArr));
  }
